...
void TBinaryTree::AddTNode(shared_ptr<TNode> newTNode, shared_ptr <TNode> root) {
...
void TBinaryTree::WalkTree(shared_ptr<TNode> n, int count) {
    if(n == nullptr) return;
...
//    return look;
//}
//void TBinaryTree::DeleteTNode(shared_ptr<TNode>,shared_ptr<TNode> parentTNode) {
//    TNode * temp;
//
...
bool TBinaryTree::InsertInTree(Item i) {
    shared_ptr <TNode> newNode(new TNode(i));
    if ( root == nullptr) {
        root = newNode;
        return true;
    } else {
